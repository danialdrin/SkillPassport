...
class ExamService:
    async def create_exam_quiz(self, user_id_str: str, resource_id_str: str) -> Dict[str, Any]:
        ...
        resource = await resources_coll.find_one({"_id": resource_id, "user_id": user_id})
        if not resource:
            raise Exception("Resource not found")

        analysis = await analyses_coll.find_one({"resource_id": resource_id}, sort=[("created_at", -1)])
        if not analysis:
    ...
    async def start_interview(self, user_id_str: str, resource_id_str: str) -> Dict[str, Any]:
        ...
        for concept in concepts:
            first_node = await skill_nodes_coll.find_one({"name": concept.get("name", "").strip().lower()})
            if first_node:
                break
        ...
        doc = {
            ...
            "current_node_id": first_node["_id"] if first_node else None,
            "status": "in_progress",
            "created_at": datetime.now(timezone.utc)
        }
    ...
    async def answer_interview_turn(self, user_id_str: str, session_id_str: str, answer_text: str) -> Dict[str, Any]:
        ...
        node_id_str = str(node_id) if node_id else str(ObjectId())
        ...
        if turn_idx >= 5:
            ...
            analysis = await analyses_coll.find_one({"resource_id": session["resource_id"]}, sort=[("created_at", -1)])
            ...
            target_name = next_json.get("target_concept", "").strip().lower()
            next_node = await get_collection("skill_nodes").find_one({"name": target_name}) if target_name else None
        ...
        await interviews_coll.update_one(
            {"_id": session_id},
            {
                ...
                "$set": {
                    ...
                    "current_node_id": next_node["_id"] if next_node else node_id,
                    "status": status_str,
