...
NON_EMPTY_ANALYSIS_KEYS = REQUIRED_ANALYSIS_KEYS
...
If the content is empty, too short, unavailable, or insufficient for reliable source-specific analysis, do not omit fields and do not return empty arrays. Instead, return a complete illustrative mock analysis using clearly labeled values such as "Mock topic based on insufficient source context". Populate every required array with at least one valid item, label the result as mock/illustrative, and do not claim unsupported facts about the original source.
"""
...
Correct the JSON syntax and include every required field with a non-empty array value.
If the source content is insufficient, populate every required array with at least one clearly labeled mock/illustrative item. Never return omitted fields or empty required arrays, and do not invent unsupported source-specific facts.
"""
        ...
        try:
            ...
            for attempt_prompt in (prompt, JSON_CORRECTION_PROMPT):
                try:
                    ...
                    empty_keys = sorted(
                        key for key in NON_EMPTY_ANALYSIS_KEYS
                        if not isinstance(json_res.get(key), list) or not json_res[key]
                    )
                    if empty_keys:
                        raise ValueError(
                            f"Strong Analysis response contains empty required fields: {empty_keys}"
                        )
            ...
            user_error = str(e)
            if "Strong Analysis JSON validation failed" in user_error:
                user_error = "Strong Analysis could not produce a valid structured result. Please retry."
            await jobs_coll.update_one(
                {"_id": ObjectId(job_id_str)},
                {"$set": {
                    "status": "failed",
