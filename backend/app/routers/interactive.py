...
async def get_analysis_for_resource(resource_id_str: str, user_id_str: str):
    ...
    resource = await resources_coll.find_one({"_id": r_id, "user_id": ObjectId(user_id_str)})
    if not resource:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Resource not found")

    analysis = await analyses_coll.find_one({"resource_id": r_id}, sort=[("created_at", -1)])
    if not analysis:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Resource not analyzed yet. Run Strong Analysis first.")

    return str(analysis["_id"]), analysis.get("version", 1), analysis.get("transcript_or_text", ""), resource.get("title", "")
...
async def get_summary(
    ...
    analysis_id_str, version, text, title = await get_analysis_for_resource(resource_id, current_user["_id"])
    ...
    prompt = SUMMARY_PROMPT.format(content=f"Title: {title}\n\n{text[:4000]}")
...
async def get_flashcards(
    ...
    prompt = FLASHCARD_PROMPT.format(content=f"Title: {title}\n\n{text[:4000]}")
    res = await llm_service.call_llm(prompt, expect_json=True)
...
async def get_practice_quiz(
    ...
    prompt = PRACTICE_QUIZ_PROMPT.format(content=f"Title: {title}\n\n{text[:4000]}")
    res = await llm_service.call_llm(prompt, expect_json=True)
