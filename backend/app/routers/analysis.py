...
@router.get("/resources/{resource_id}/analysis", response_model=AnalysisResponse)
async def get_latest_resource_analysis(
    resource_id: str,
    current_user: dict = Depends(get_current_user)
):
    try:
    ...
    analysis = await analyses_coll.find_one(
        {"resource_id": r_id},
        sort=[("created_at", -1)],
    )
    if not analysis:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Resource analysis not found")

    extracted = StrongAnalysisResultSchema.model_validate(analysis.get("extracted_data", {}))
    return AnalysisResponse(
        analysis_id=str(analysis["_id"]),
        resource_id=str(analysis["resource_id"]),
        transcript_or_text=analysis.get("transcript_or_text", ""),
